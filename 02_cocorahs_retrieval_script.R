...
cocorahs_data_raw_fin <- cocorahs_data_raw %>%
  dplyr::mutate(time_og = hm(str_sub(time, start = 1, end = 5)),
  ...
  dplyr::mutate(datetime_et = as.character(ymd_hms(paste0(date, " ", time_fix_fin), tz = "EST"))) %>%
  na.omit() %>% # delete NA entries
  dplyr::select(date, time, datetime_et, station_id:precip_in) %>%
  dplyr::arrange(datetime_et)
...
cocorahs_metadata <- cocorahs_data_raw_fin %>%
  ...
  dplyr::select(location_id = station_id, location_id_code = station_id, location_name = station_name, latitude_degrees_north = lat, longitude_degrees_east = long) %>%
  dplyr::distinct_all() %>%
  # add columns so this is compatible with NC SCO API outputs
  dplyr::mutate(network_type = "CoCoRaHS",
                city = NA, 
                county = NA, 
                state = "NC", 
                elevation_feet = NA, 
  ...
  dplyr::select(location_id, location_id_code, network_type, location_name,city:state, latitude_degrees_north, longitude_degrees_east, elevation_feet:obtypes_available)


# ---- 5. separate out data (to combine with clouds api data) ----
cocorahs_data <- cocorahs_data_raw_fin %>%
  dplyr::select(location_id = station_id, datetime_et, precip_in) %>%
  # add columns so this is compatible with NC SCO API outputs
  dplyr::mutate(var = "precip",
                value = precip_in,
                unit = "in",
                score = NA,
                nettype = NA,
                ...
                obnum = NA,
                value_accum = NA) %>%
  dplyr::select(location_id, datetime_et, var:value_accum)
