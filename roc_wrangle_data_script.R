...
for (i in 288:dim(data_available)[1]) {
  ...
  if (status == "available") {
    ...
    for (j in 1:length(valid_period_list)) {
      ...
      for (k in 1:num_obs_stations) {
        # save location name
        temp_loc_id <- as.character(hist_precip_metadata_albers_sel$loc_id[k])
        
        # save cmu name
        temp_cmu_name <- as.character(hist_precip_metadata_albers_sel$cmu_name[k])
